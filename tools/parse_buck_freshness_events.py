...
def parse(path: Path, target: str, require_execution: bool = True) -> dict:
    ...
    if not isinstance(value, dict) or set(value) != {"target", "events", "adapter_markers"}:
        raise SystemExit("freshness evidence must contain exactly target, events, adapter_markers")
    if value["target"] != target:
        raise SystemExit("freshness evidence target identity does not match invocation")
    events = value["events"]
    markers = value["adapter_markers"]
    if not isinstance(events, list) or not isinstance(markers, list):
        raise SystemExit("freshness events and adapter markers must be lists")
    ...
    if not markers or any(marker != "buck2-nextest-contract: dispatch=deferred-nextest-suite" for marker in markers):
        raise SystemExit("freshness evidence is missing the authoritative adapter execution marker")
