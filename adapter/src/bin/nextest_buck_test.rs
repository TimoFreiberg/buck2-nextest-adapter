...
impl RunnerError {
    ...
    fn infrastructure(message: impl Into<String>) -> Self {
        Self::new(INFRASTRUCTURE_STATUS, message)
    }
}
    ...
    while index < argv.len() {
        ...
        if option == OsStr::new("--manifest") {
            if args.manifest.is_some() {
                fail("--manifest specified more than once");
            }
            args.manifest = Some(PathBuf::from(take_value(&argv, &mut index, "--manifest")));
        } else if option == OsStr::new("--record-count") {
            if args.record_count.is_some() {
                fail("--record-count specified more than once");
            }
            args.record_count = Some(
                take_value(&argv, &mut index, "--record-count")
                    .to_str()
                    .and_then(|v| v.parse().ok())
                    .unwrap_or_else(|| fail("invalid --record-count")),
            );
        } else if option == OsStr::new("--declared-input") {
            args.declared_inputs.push(PathBuf::from(take_value(
                &argv,
                &mut index,
                "--declared-input",
            )));
        } else if option == OsStr::new("--suite-env") {
            let name = take_value(&argv, &mut index, "--suite-env")
                .to_str()
                .unwrap_or_else(|| fail("suite environment name is not UTF-8"))
                .to_owned();
            let value = take_value(&argv, &mut index, "--suite-env")
                .to_str()
                .unwrap_or_else(|| fail("suite environment value is not UTF-8"))
                .to_owned();
            if args.suite_env.insert(name, value).is_some() {
                fail("suite environment variable specified more than once");
            }
        } else if option == OsStr::new("--cargo-nextest-argv") {
            if args.nextest.is_some() {
                fail("--cargo-nextest-argv specified more than once");
            }
            args.nextest = Some(take_vec(&argv, &mut index, "--cargo-nextest-argv"));
        } else if option == OsStr::new("--bundle-json") {
            if args.bundle_json.is_some() {
                fail("--bundle-json specified more than once");
            }
            args.bundle_json = Some(
                take_value(&argv, &mut index, "--bundle-json")
                    .to_str()
                    .unwrap_or_else(|| fail("bundle JSON is not UTF-8"))
                    .into(),
            );
        } else if option == OsStr::new("--bundle-resources") {
            if args.bundle_resources_seen {
                fail("--bundle-resources specified more than once");
            }
            args.bundle_resources_seen = true;
            index += 1;
            while let Some(value) = argv.get(index) {
                if value == OsStr::new("--end-bundle-resources") {
                    break;
                }
            ...
            if index == argv.len() {
                fail("--bundle-resources requires --end-bundle-resources");
            }
        } else if option == OsStr::new("--profile") {
            if args.profile_seen {
                fail("--profile specified more than once");
            }
            args.profile_seen = true;
            args.profile = take_value(&argv, &mut index, "--profile")
                .to_str()
                .unwrap_or_else(|| fail("profile is not UTF-8"))
                .into();
        } else if option == OsStr::new("--filter") {
            if args.filter_seen {
                fail("--filter specified more than once");
            }
            args.filter_seen = true;
            args.filter = take_value(&argv, &mut index, "--filter")
                .to_str()
                .unwrap_or_else(|| fail("filter is not UTF-8"))
                .into();
        } else if option == OsStr::new("--no-tests") {
            if args.no_tests_seen {
                fail("--no-tests specified more than once");
            }
            args.no_tests_seen = true;
            args.no_tests = take_value(&argv, &mut index, "--no-tests")
                .to_str()
                .unwrap_or_else(|| fail("no-tests is not UTF-8"))
                .into();
        } else if option == OsStr::new("--report-skipped") {
            if args.report_skipped_seen {
                fail("--report-skipped specified more than once");
            }
            args.report_skipped_seen = true;
            args.report_skipped = take_value(&argv, &mut index, "--report-skipped")
                .to_str()
                .unwrap_or_else(|| fail("report-skipped is not UTF-8"))
                .into();
        } else if option == OsStr::new("--timeout-seconds") {
            if args.timeout_seen {
                fail("--timeout-seconds specified more than once");
            }
            args.timeout_seen = true;
            args.timeout = take_value(&argv, &mut index, "--timeout-seconds")
                .to_str()
                .and_then(|v| v.parse().ok())
                .unwrap_or_else(|| fail("invalid --timeout-seconds"));
        } else {
...
fn validate_suite_environment(environment: &BTreeMap<String, String>) -> Result<(), String> {
    ...
    for (name, value) in environment {
        ...
        if !valid_name || name.starts_with("BUCK2_NEXTEST_") || reserved.contains(&name.as_str()) {
            return Err(format!(
                "suite environment name is reserved or invalid: {name}"
            ));
        }
        if value.contains('\0') {
...
fn validate_controls(args: &Args) -> Result<(), String> {
    if args.manifest.is_none()
        || args.record_count.is_none()
        || args.nextest.as_ref().is_none_or(Vec::is_empty)
    {
        return Err(
            "manifest, record-count, and non-empty declared launcher argv are required".into(),
        );
    }
    ...
    {
        return Err("profile must match [A-Za-z0-9][A-Za-z0-9_-]*".into());
    }
    if !matches!(args.no_tests.as_str(), "auto" | "pass" | "warn" | "fail") {
        return Err("no-tests must be auto, pass, warn, or fail".into());
    }
    if !matches!(args.report_skipped.as_str(), "default" | "ignored") {
        return Err("report-skipped must be default or ignored".into());
    }
    if args.timeout > 86400 {
        return Err("timeout-seconds must be between 0 and 86400".into());
    }
    Ok(())
}
fn empty_declared_directory(path: &Path, cwd: &Path) -> Result<PathBuf, String> {
    ...
    let metadata = fs::symlink_metadata(&path)
        .map_err(|e| format!("BUCK2_NEXTEST_JUNIT_DIR cannot be inspected: {e}"))?;
    if metadata.file_type().is_symlink() || !metadata.file_type().is_dir() {
        return Err("BUCK2_NEXTEST_JUNIT_DIR must be a real directory".into());
    }
    if fs::read_dir(&path)
        ...
        .is_some()
    {
        return Err("BUCK2_NEXTEST_JUNIT_DIR must be empty before dispatch".into());
    }
    Ok(path)
}
fn ensure_directory_without_symlinks(path: &Path) -> Result<(), String> {
    if !path.is_absolute() {
        return Err(format!(
            "scratch parent must be absolute: {}",
            path.display()
        ));
    }
    ...
    for component in path.components() {
        ...
        match fs::symlink_metadata(&current) {
            Ok(metadata) => {
                if metadata.file_type().is_symlink() || !metadata.file_type().is_dir() {
                    return Err(format!(
                        "scratch parent contains an unsafe directory: {}",
                        current.display()
                    ));
                }
            }
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                ...
                if metadata.file_type().is_symlink() || !metadata.file_type().is_dir() {
                    return Err(format!(
                        "scratch parent was replaced by an unsafe directory: {}",
                        current.display()
                    ));
                }
            }
            Err(error) => {
                return Err(format!(
                    "could not inspect scratch parent {}: {error}",
                    current.display()
                ))
            }
        }
...
fn private_root(cwd: &Path) -> Result<PathBuf, String> {
    ...
    for attempt in 0..1000 {
        ...
        match fs::create_dir(&root) {
            ...
            Err(error) => return Err(format!("could not create private scratch root: {error}")),
        }
    }
...
fn source_for(source: &str, declared: &[PathBuf], cwd: &Path) -> Result<PathBuf, String> {
    ...
    if matches.len() != 1 {
        return Err(format!(
            "declared input does not exactly map to source: {source}"
        ));
    }
...
fn parse_bundle(
...
) -> Result<(BTreeMap<String, String>, Vec<(PathBuf, PathBuf)>), String> {
    let Some(raw) = raw else {
        return Ok((BTreeMap::new(), Vec::new()));
    };
    let value: Value =
        serde_json::from_str(raw).map_err(|e| format!("invalid bundle JSON: {e}"))?;
    ...
    if object.keys().map(String::as_str).collect::<BTreeSet<_>>() != expected {
        return Err("bundle JSON has unexpected fields".into());
    }
    if object.get("bundle_version").and_then(Value::as_u64) != Some(1) {
    ...
    if platform.is_empty() || platform.chars().any(char::is_whitespace) {
        return Err("bundle_platform is invalid".into());
    }
    let resources = object
        .get("bundle_resources")
        .and_then(Value::as_array)
        .ok_or("bundle_resources must be a list")?;
    if resources.len() != declared.len() {
    ...
    for resource in resources {
        let item = resource
            .as_object()
            .ok_or("bundle resource must be an object")?;
        let keys: BTreeSet<&str> = ["digest", "path", "source"].into_iter().collect();
        if item.keys().map(String::as_str).collect::<BTreeSet<_>>() != keys {
            return Err("bundle resource has unexpected fields".into());
        }
        ...
        buck2_nextest_adapter_contract::nextest_v2::safe_relative(
            source,
        ...
        if destination == "workspace"
            ...
            || destination == "meta"
            || destination.starts_with("meta/")
            || destination == "target"
            || destination.starts_with("target/")
            || destination == "manifest.json"
            || destination.starts_with("manifest.json/")
            || manifest_paths
                .iter()
                .any(|path| paths_overlap(path, destination))
        {
            return Err(
                "bundle resource path is adapter-owned or overlaps a declared test artifact".into(),
            );
        }
        if !sources.insert(source) || !paths.insert(destination) {
        ...
        if parts.next() != Some("sha256") {
            return Err("bundle resource digest must use sha256:<hex>:<size>".into());
        }
        let hash = parts
            .next()
            .ok_or("bundle resource digest is missing hash")?;
        let size = parts
            .next()
            .and_then(|v| v.parse::<u64>().ok())
            .ok_or("bundle resource digest has invalid size")?;
        ...
        let actual = buck2_nextest_adapter_contract::nextest_v2::sha256_digest(&input)?;
        if metadata.len() != size || !actual.eq_ignore_ascii_case(hash) {
            return Err(format!("bundle resource digest mismatch: {source}"));
        }
    ...
    for item in object["bundle_environment"]
        .as_array()
        .ok_or("bundle_environment must be a list")?
    {
        let item = item
            .as_object()
            .ok_or("bundle environment must be an object")?;
        let name = item
            .get("name")
            .and_then(Value::as_str)
        ...
        if !name.bytes().enumerate().all(|(i, b)| {
            ...
            || !matches!(kind, "literal" | "relative_path")
            || value.contains('\0')
            || (kind == "relative_path"
                && (buck2_nextest_adapter_contract::nextest_v2::safe_relative(
                    value,
                ...
                .is_err()
                    || !declared_destinations.contains(value)))
            || environment
                .insert(
                    name.to_owned(),
                    if kind == "relative_path" {
                        abs(&root.join(value), cwd).to_string_lossy().into_owned()
                    } else {
                        value.to_owned()
                    },
                )
                .is_some()
        {
            return Err("invalid or duplicate bundle environment".into());
        }
    }
...
fn build_command(
    base: &[OsString],
    subcommand: &str,
    generated: &buck2_nextest_adapter_contract::nextest_v2::Synthesized,
    root: &Path,
    args: &Args,
    bundle_env: &BTreeMap<String, String>,
) -> Result<Command, String> {
    let mut argv = base.to_vec();
    argv.push("nextest".into());
    argv.push(subcommand.into());
    ...
    command
        .arg("--cargo-metadata")
        .arg(&generated.cargo_metadata)
        .arg("--binaries-metadata")
        .arg(&generated.binaries_metadata)
        .arg("--target-dir-remap")
        .arg(&generated.target)
        .arg("--build-dir-remap")
        .arg(&generated.target)
        .arg("--workspace-remap")
        .arg(&generated.workspace)
        .arg("--config-file")
        .arg(&generated.config_file);
    if subcommand == "list" {
...
fn run_capture(mut command: Command) -> Result<(i32, Vec<u8>, Vec<u8>), ProcessFailure> {
    ...
    let mut child = command
        .spawn()
        .map_err(|e| format!("could not spawn setup command: {e}"))?;
    ...
    match buck2_nextest_adapter_contract::nextest_v2::collect_process_groups(root_pid) {
        ...
        Err(error) => {
            let signal_error =
                buck2_nextest_adapter_contract::nextest_v2::send_group(root_pid, 9).err();
            let wait_error = child.wait().err().map(|error| error.to_string());
            let drain_error = drain_process_groups(&groups).err();
            let mut message = format!(
                "could not inspect setup process group: {error}; escalated known process group"
            );
            if let Some(error) = signal_error {
                message.push_str(&format!("; escalation failed: {error}"));
            }
            if let Some(error) = wait_error {
                message.push_str(&format!("; leader wait failed: {error}"));
            }
            if let Some(error) = drain_error {
                message.push_str(&format!("; process-group drain failed: {error}"));
            }
            return Err(ProcessFailure::retained(message));
        }
    }
    let stdout_pipe = child.stdout.take().map(|mut pipe| {
        thread::spawn(move || {
    ...
    let stderr_pipe = child.stderr.take().map(|mut pipe| {
        thread::spawn(move || {
            let mut output = Vec::new();
            pipe.read_to_end(&mut output)
                .map(|_| output)
    ...
    loop {
        if let Some(status) = child.try_wait().map_err(|e| e.to_string())? {
            ...
            drain_process_groups(&groups).map_err(|error| {
                ProcessFailure::retained(format!(
                    "setup process groups did not become quiescent: {error}"
                ))
            })?;
            ...
            if let Some(signal) = signal {
                return Ok((signal.status(), stdout, stderr));
            }
        ...
        if let Some(observed) = guard.observed() {
            if signal.is_none() {
                ...
                send_process_tree(root_pid, &mut groups, observed.0).map_err(|error| {
                    ProcessFailure::retained(format!(
                        "could not signal setup process group tree: {error}"
                    ))
                })?;
                signal_sent_at = Some(std::time::Instant::now());
            } else if !kill_sent
                && signal_sent_at.is_some_and(|sent| sent.elapsed() >= Duration::from_secs(5))
            {
                send_process_tree(root_pid, &mut groups, 9).map_err(|error| {
                    ProcessFailure::retained(format!(
                        "could not kill setup process group tree: {error}"
                    ))
                })?;
                kill_sent = true;
            }
...
fn version_matches(text: &str) -> bool {
    text.lines().any(|line| {
        let line = line.trim();
        line == RUNNER_VERSION
            || (line.starts_with(RUNNER_VERSION) && line[RUNNER_VERSION.len()..].starts_with(" ("))
    })
}
fn validate_list(
    stdout: &[u8],
    ...
    let mut documents = text
        ...
        .map(|line| {
            serde_json::from_str::<Value>(line)
                .map_err(|e| format!("nextest list emitted invalid JSON: {e}"))
        })
        .collect::<Result<Vec<_>, _>>()?;
    if documents.len() != 1 {
        return Err(format!(
            "nextest list must emit exactly one JSON document, got {}",
            documents.len()
        ));
    }
    ...
    for (id, suite) in suites {
        let object = suite
            .as_object()
            .ok_or_else(|| format!("nextest list suite {id} is not an object"))?;
        let reported_id = object
            .get("binary-id")
            .and_then(Value::as_str)
            .ok_or_else(|| format!("nextest list suite {id} has no binary-id"))?;
        if reported_id != id {
            return Err(format!(
...
fn run_supervised(
    mut command: Command,
) -> Result<(i32, Option<ObservedSignal>, Vec<u8>, Vec<u8>), ProcessFailure> {
    set_process_group(&mut command);
    let guard = SignalGuard::install()?;
    let mut child = command
        .spawn()
        .map_err(|e| format!("could not spawn nextest run: {e}"))?;
    let root_pid = child.id() as i32;
    if root_pid <= 0 {
        let _ = child.kill();
        let _ = child.wait();
    ...
    match buck2_nextest_adapter_contract::nextest_v2::collect_process_groups(root_pid) {
        ...
        Err(error) => {
            ...
            let drain_error = drain_process_groups(&groups).err();
            let mut message = format!(
                "could not inspect nextest process group: {error}; escalated known process group"
            );
            if let Some(error) = signal_error {
                message.push_str(&format!("; escalation failed: {error}"));
            }
    ...
    loop {
        if let Some(status) = child.try_wait().map_err(|e| e.to_string())? {
            ...
            drain_process_groups(&groups).map_err(|error| {
                ProcessFailure::retained(format!(
                    "nextest process groups did not become quiescent: {error}"
                ))
            })?;
            let stdout = stdout_pipe
                .map(|pipe| {
        ...
        if let Some(observed) = guard.observed() {
            if signal.is_none() {
                ...
                send_process_tree(root_pid, &mut groups, observed.0).map_err(|error| {
                    ProcessFailure::retained(format!(
                        "could not signal nextest process group tree: {error}"
                    ))
                })?;
                signal_sent_at = Some(std::time::Instant::now());
            } else if !kill_sent
                && signal_sent_at.is_some_and(|sent| sent.elapsed() >= Duration::from_secs(5))
            {
                send_process_tree(root_pid, &mut groups, 9).map_err(|error| {
                    ProcessFailure::retained(format!(
                        "could not kill nextest process group tree: {error}"
                    ))
                })?;
                kill_sent = true;
            }
...
fn execute(args: Args) -> Result<i32, RunnerError> {
    ...
    let declared_dir = PathBuf::from(
        env::var_os("BUCK2_NEXTEST_JUNIT_DIR").ok_or("BUCK2_NEXTEST_JUNIT_DIR is required")?,
    );
    ...
    if manifest.records.len() != args.record_count.unwrap() {
        return Err("manifest record count does not match --record-count".into());
    }
    let inputs: Vec<_> = args
        .declared_inputs
        .iter()
        .map(|path| abs(path, &cwd))
        .collect();
    for input in &inputs {
        buck2_nextest_adapter_contract::nextest_v2::regular(input, false)?;
    }
    ...
    let execution = (|| -> Result<(i32, Option<PathBuf>), ProcessFailure> {
        ...
        for (source, destination) in bundle_pairs {
            buck2_nextest_adapter_contract::nextest_v2::copy_regular(&source, &destination, false)?;
        }
        let mut version_command = command_from(&version)?;
        version_command.current_dir(&generated.workspace);
        sanitized_env(
            &mut version_command,
            &root,
            &args.suite_env,
        ...
        let (version_status, version_stdout, version_stderr) = run_capture(version_command)
            .map_err(|error| ProcessFailure {
                ...
                retain_scratch: error.retain_scratch,
            })?;
        let version_text = String::from_utf8_lossy(&version_stdout).to_string()
            + &String::from_utf8_lossy(&version_stderr);
        if version_status != 0 || !version_matches(&version_text) {
            return Err(format!("version boundary failed: expected {RUNNER_VERSION} with optional parenthesized build details, got {}", version_text.trim()).into());
        }
        let list = build_command(base, "list", &generated, &root, &args, &bundle_env)?;
        let (list_status, list_stdout, list_stderr) =
            run_capture(list).map_err(|error| ProcessFailure {
                message: error.message,
                retain_scratch: error.retain_scratch,
            })?;
        if list_status != 0 {
            return Err(format!(
                "list boundary failed with {list_status}: {}",
                String::from_utf8_lossy(&list_stderr)
            )
            .into());
        }
        validate_list(&list_stdout, &generated.list_expectations)?;
        let run = build_command(base, "run", &generated, &root, &args, &bundle_env)?;
        let (raw, signal, stdout, stderr) =
            run_supervised(run).map_err(|error| ProcessFailure {
                message: error.message,
                retain_scratch: error.retain_scratch,
            })?;
        if let Some(signal) = signal {
            eprintln!("buck2-nextest-adapter: terminated by signal {}", signal.0);
            return Ok((signal.status(), None));
        }
        if !stdout.is_empty() {
    ...
    if retain_scratch {
        ...
        return Err(RunnerError::new(
            2,
            format!(
                "process lifecycle failed; retained scratch root {}",
                root.display()
            ),
        ));
    }
    match fs::remove_dir_all(&root) {
        ...
        Err(error) => Err(RunnerError::new(
            2,
            format!(
                "scratch cleanup failed; retained root {}: {error}",
                root.display()
            ),
        )),
    }
...
mod tests {
    use super::*;
    #[test]
    fn controls_reject_invalid_values() {
        let args = Args {
    ...
    fn source_resolution_prefers_exact_declared_path() {
        let declared = vec![
            PathBuf::from("remapped/bin/test"),
            PathBuf::from("other/test"),
        ];
        assert_eq!(
            source_for("remapped/bin/test", &declared, Path::new("/tmp")),
            Ok(PathBuf::from("/tmp/remapped/bin/test"))
        );
    }
    #[test]
