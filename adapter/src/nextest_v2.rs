...
use crate::{digest::sha256_file, manifest_v2::{GenericRecord, ManifestV2}, report::validate_report_xml};
use serde_json::{json, Map, Value};
use signal_hook::{consts::{SIGHUP, SIGINT, SIGTERM}, iterator::{Handle, Signals}};
use std::{collections::{BTreeMap, BTreeSet}, env, ffi::OsString, fs::{self, File, OpenOptions}, io::Write, path::{Path, PathBuf}, process::{Command, ExitStatus}, sync::{atomic::{AtomicI32, Ordering}, Arc}, thread, time::{Duration, Instant}};


#[cfg(unix)]
use std::{os::raw::c_int, os::unix::fs::PermissionsExt};
#[cfg(unix)]
unsafe extern "C" { fn kill(pid: c_int, signal: c_int) -> c_int; }
...
pub fn safe_relative(value: &str, name: &str) -> Result<(), String> {
    if value.is_empty() || value.starts_with('/') || value.contains('\\') || value.contains('\0') || value.split('/').any(|p| p.is_empty() || p == "." || p == "..") {
...
pub fn regular(path: &Path, executable: bool) -> Result<(), String> {
    let metadata = fs::symlink_metadata(path).map_err(|e| format!("cannot inspect {}: {e}", path.display()))?;
    if !metadata.file_type().is_file() { return Err(format!("{} is not a regular file", path.display())); }
    if executable && !is_executable(path) { return Err(format!("{} is not executable", path.display())); }
    File::open(path).map_err(|e| format!("{} is not readable: {e}", path.display()))?;
    Ok(())
}

#[cfg(unix)] fn is_executable(path: &Path) -> bool { fs::metadata(path).map(|m| m.permissions().mode() & 0o111 != 0).unwrap_or(false) }
#[cfg(not(unix))] fn is_executable(path: &Path) -> bool { fs::metadata(path).is_ok() }

pub fn ensure_dir(path: &Path) -> Result<(), String> {
    if let Ok(metadata) = fs::symlink_metadata(path) {
        if metadata.file_type().is_symlink() || !metadata.file_type().is_dir() { return Err(format!("{} is not a real directory", path.display())); }
        return Ok(());
    }
    fs::create_dir_all(path).map_err(|e| format!("could not create {}: {e}", path.display()))?;
    let metadata = fs::symlink_metadata(path).map_err(|e| e.to_string())?;
    if metadata.file_type().is_symlink() || !metadata.file_type().is_dir() { return Err(format!("{} was replaced by a non-directory", path.display())); }
    Ok(())
}

pub fn copy_regular(source: &Path, destination: &Path, executable: bool) -> Result<(), String> {
    regular(source, false)?;
    if let Some(parent) = destination.parent() { ensure_dir(parent)?; }
    if let Ok(metadata) = fs::symlink_metadata(destination) { if metadata.file_type().is_symlink() || !metadata.file_type().is_file() { return Err(format!("staging destination is unsafe: {}", destination.display())); } }
    fs::copy(source, destination).map_err(|e| format!("could not stage {}: {e}", source.display()))?;
    if executable {
        #[cfg(unix)] {
            let mut permissions = fs::metadata(destination).map_err(|e| e.to_string())?.permissions();
...
pub fn synthesize(root: &Path, manifest: &ManifestV2, profile: &str, report_skipped: &str, timeout: u64) -> Result<Synthesized, String> {
    let workspace = root.join("workspace");
    let target = root.join("target");
    let meta = root.join("meta");
    ensure_dir(&workspace)?; ensure_dir(&meta)?; ensure_dir(&target.join("debug"))?;
    ensure_dir(&workspace.join(".config"))?;
    let mut packages: BTreeMap<String, (String, String, Vec<(&GenericRecord, String)>)> = BTreeMap::new();
    ...
    for record in &manifest.records {
        ...
        let entry = packages.entry(record.package_identity.clone()).or_insert_with(|| (package_name, package_dir, Vec::new()));
    ...
    for (_identity, (package_name, _package_dir, records)) in &packages {
        ...
        let package_id = format!("path+file://{}#{}@0.1.0", package_root.display(), package_name);
    ...
    let package_ids: Vec<Value> = cargo_packages.iter().map(|package| package["id"].clone()).collect();
    ...
    json_write(&meta.join("cargo-metadata.json"), &cargo)?; json_write(&meta.join("binaries-metadata.json"), &binary)?;
    let junit = target.join("nextest").join(profile).join("junit.xml");
    let mut config = format!("[profile.{profile}]\n");
    if report_skipped == "ignored" { config.push_str("report-skipped = \"ignored\"\n"); }
    if timeout != 0 { config.push_str(&format!("slow-timeout = {{ period = \"{timeout}s\", terminate-after = 1, grace-period = \"0s\" }}\n")); }
    let junit_path = junit.to_str().ok_or("generated JUnit path is not valid UTF-8")?;
    ...
    Ok(Synthesized { workspace: workspace.clone(), target: target.clone(), cargo_metadata: meta.join("cargo-metadata.json"), binaries_metadata: meta.join("binaries-metadata.json"), config_file: workspace.join(".config/nextest.toml"), junit, binary_ids: ids, list_expectations, staged_binaries })
}
...
pub fn publish_junit(source: &Path, declared_dir: &Path) -> Result<(), String> {
    ...
    if metadata.file_type().is_symlink() || !metadata.file_type().is_dir() { return Err("declared JUnit directory is unsafe".into()); }
    ...
    for entry in fs::read_dir(declared_dir).map_err(|e| e.to_string())? { let entry = entry.map_err(|e| e.to_string())?; return Err(format!("declared JUnit directory is not empty: {}", entry.path().display())); }
    ...
    #[cfg(unix)] { file.set_permissions(fs::Permissions::from_mode(0o600)).map_err(|e| e.to_string())?; }
    ...
    if fs::symlink_metadata(&destination).is_ok() { let _ = fs::remove_file(&temporary); return Err("declared JUnit destination already exists".into()); }
    fs::rename(&temporary, &destination).map_err(|e| e.to_string())?;
    File::open(declared_dir).and_then(|directory| directory.sync_all()).map_err(|e| e.to_string())
}
...
pub fn sanitized_env(command: &mut Command, root: &Path, suite_env: &BTreeMap<String, String>, bundle_env: &BTreeMap<String, String>, workspace: &Path, target: &Path) {
    command.env_clear();
    for (key, value) in bundle_env { command.env(key, value); }
    for (key, value) in suite_env { command.env(key, value); }
    ...
    command.env("PATH", "").env("HOME", root.join("home")).env("CARGO_HOME", root.join("cargo-home")).env("TMPDIR", root.join("tmp")).env("CARGO_TARGET_DIR", target).env("CARGO_MANIFEST_DIR", workspace).env("CARGO_NET_OFFLINE", "true").env("CARGO_NET_GIT_FETCH_WITH_CLI", "false");
}

#[derive(Debug, Copy, Clone, Eq, PartialEq)] pub struct ObservedSignal(pub i32);
impl ObservedSignal { pub fn status(self) -> i32 { 128 + self.0 } }
pub struct SignalGuard { first: Arc<AtomicI32>, handle: Handle, watcher: Option<thread::JoinHandle<()>> }
impl SignalGuard { pub fn install() -> Result<Self, String> { let first = Arc::new(AtomicI32::new(0)); let state = first.clone(); let mut signals = Signals::new([SIGHUP, SIGINT, SIGTERM]).map_err(|e| e.to_string())?; let handle = signals.handle(); let watcher = thread::spawn(move || { for signal in signals.forever() { if matches!(signal, SIGHUP | SIGINT | SIGTERM) { let _ = state.compare_exchange(0, signal, Ordering::SeqCst, Ordering::SeqCst); break; } } }); Ok(Self { first, handle, watcher: Some(watcher) }) } pub fn observed(&self) -> Option<ObservedSignal> { match self.first.load(Ordering::SeqCst) { 0 => None, signal => Some(ObservedSignal(signal)) } } }
impl Drop for SignalGuard { fn drop(&mut self) { self.handle.close(); if let Some(thread) = self.watcher.take() { let _ = thread.join(); } } }
...
pub fn collect_process_groups(root: i32) -> Result<BTreeSet<i32>, String> {
    ...
    for line in text.lines() {
        ...
        let pid = fields[0].parse::<i32>().map_err(|_| format!("malformed process id: {:?}", fields[0]))?;
        let ppid = fields[1].parse::<i32>().map_err(|_| format!("malformed parent process id: {:?}", fields[1]))?;
        let pgid = fields[2].parse::<i32>().map_err(|_| format!("malformed process group id: {:?}", fields[2]))?;
        if pid <= 0 || pgid <= 0 {
...
pub fn drain_process_groups(groups: &BTreeSet<i32>) -> Result<(), String> {
    ...
    let groups: Vec<_> = groups.iter().copied().filter(|group| *group != own_group).collect();
    ...
    while Instant::now() < deadline {
        let live = groups.iter().copied().any(|group| group_exists(group).unwrap_or(true));
        if !live {
            return first_error.map_or(Ok(()), Err);
        }
        thread::sleep(Duration::from_millis(20));
...
#[cfg(not(unix))]
fn group_exists(_pgid: i32) -> Result<bool, String> { Err("process groups are unsupported".into()) }

pub fn status_code(status: ExitStatus) -> Result<i32, String> { if let Some(code) = status.code() { return Ok(code); } #[cfg(unix)] { use std::os::unix::process::ExitStatusExt; if let Some(signal) = status.signal() { return Ok(128 + signal); } } Err("child has no usable exit status".into()) }

pub fn sha256_digest(path: &Path) -> Result<String, String> { sha256_file(path).map_err(|e| e.to_string()) }

#[cfg(test)]
mod tests {
    use super::*;
    #[test] fn generated_names_are_stable_and_injective() { assert_eq!(generated_name("buck2", "b2n1:p=a;o=//:x;b=one"), "buck2_62_32_6e_31_3a_70_3d_61_3b_6f_3d_2f_2f_3a_78_3b_62_3d_6f_6e_65"); assert_ne!(generated_name("buck2", "a_5f"), generated_name("buck2", "a%5f")); }
    #[test] fn safe_paths_reject_traversal() { assert!(safe_relative("../x", "cwd").is_err()); assert!(safe_relative("work/x", "cwd").is_ok()); }
    #[test]
    ...
    fn owned_process_group_drain_terminates_descendants() {
        ...
        assert!(!groups.is_empty(), "owned process group must contain its leader");
