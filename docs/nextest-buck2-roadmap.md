...
Remaining Phase 6 work is failed-run artifact semantics, persistent-record policy, stronger concurrency/cache/remote-like validation, and hermetic/remote-ready toolchains. The installed Buck2 exposes a usable repository-level `aquery --json --output-attribute '^cmd$'` command, and the standalone inspection helper verifies both configured commands and their differing profile inputs; nested invocation from `sh_test` is intentionally deferred because this test environment cannot safely run Buck inside a Buck test. Retries, groups, arbitrary profile TOML, and actual remote execution remain deferred to later phases.
